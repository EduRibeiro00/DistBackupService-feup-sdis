...
public class FileManager {
    ...
    private final static String fileOwnerInfo = "file_owner_info.data";                  /** name of the file containing information about the owner of the files */
    ...
    private final int peerId;              /** The ID of the peer of which files are being managed */
    ...
    /**
     * Stores the generated FileID hashes for each of the files it has asked to back up.
     * key = file path
    ...
    /**
     * Stores the owners of the files/chunks received.
     * key = file id
     * value = IP Address and port of the owners of the file
     */
    private ConcurrentHashMap<String, AddressRecord> fileOwnerTable;
    ...
    /**
     * Returns the file owner of the file id given
     * @param fileId id of the file
     * @return the owner's IP address and port
     */
    public AddressRecord getFileOwner(String fileId) {
        return fileOwnerTable.getOrDefault(fileId, null);
    }

    /**
     * Adds or updates the file owner of a given file id
     * @param fileId id of the file
     * @param ipAddress IP address
     * @param port Port number
     */
    public void addFileOwner(String fileId, String ipAddress, int port) {
        this.fileOwnerTable.put(fileId, new AddressRecord(ipAddress, port));
        this.saveToDirectory();
    }

    /**
     * Removes the file owner of a given file id from the table
     * @param fileId id of the file
     */
    public void removeFileOwner(String fileId) {
        this.fileOwnerTable.remove(fileId);
        this.saveToDirectory();
    }
    ...
    public boolean amFileOwner(String fileId) {
        for (String value : this.hashBackedUpFiles.values()){
    ...
    public String getDirectoryPath(String dirName) {
        return System.getProperty("user.dir") + "/src/storage/" + dirName + "/" + this.peerId + "/";
    }
    ...
    public ConcurrentHashMap<String, Integer> getChunkSizes() {
        return chunkSizes;
    }
    ...
    /**
     * Returns a concurrent hashmap storing the chunks numbers for all the files
     * @return A list with the chunks of the file
     */
    public ConcurrentHashMap<String, ConcurrentSkipListSet<Integer>> getCompleteFilesChunks() {
        return this.fileToChunks;
    }
    ...
    private void loadFromDirectory() {
        ...
        // Loading file owner table
        try {
            FileInputStream fileOwnFileIn = new FileInputStream(this.getDirectoryPath("chunks") + fileOwnerInfo);
            ObjectInputStream fileOwnObjIn = new ObjectInputStream(fileOwnFileIn);
            this.fileOwnerTable = (ConcurrentHashMap<String, AddressRecord>)fileOwnObjIn.readObject();
            fileOwnObjIn.close();
            fileOwnFileIn.close();
        } catch (Exception e) {
            this.fileOwnerTable = new ConcurrentHashMap<>();
        }
        ...
        for (Map.Entry<String, ConcurrentSkipListSet<Integer>> entry : this.fileToChunks.entrySet()) {
            ...
            for (int chunkNo : entry.getValue()) {
                ...
                if (chunk.exists()) {
                    int size = (int) chunk.length() / 1000;
                    if (size == 0)
                        size = 1;

                    this.chunkSizes.put(filename, size);
    ...
    synchronized private void saveToDirectory() {
        ...
        // Saving file owner list
        try {
            FileOutputStream fileOwnFileOut = new FileOutputStream(this.getDirectoryPath("chunks") + fileOwnerInfo);
            ObjectOutputStream fileOwnObjOut = new ObjectOutputStream(fileOwnFileOut);
            fileOwnObjOut.writeObject(this.fileOwnerTable);
            fileOwnObjOut.close();
            fileOwnFileOut.close();
        } catch (Exception ignore) {}
    }
}
